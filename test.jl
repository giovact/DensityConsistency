...
@testset "Exactness on trees" begin
    ...
    @testset "Ising open-chains $i" for i in 1:length(filenames)
        ...
        @testset "β $β" for β in βvec
            psi = DC.IsingFactors(N,β*model["h"],β*sparse(model["J"]), model["Adj"], :Fun);
            ϕ , res, it, epsv = DC.density_consistency(psi,N,verbose = false, epsconv = 1e-10)
            @test isapprox(true_moments[β][1], ϕ.µt, atol = 1e-9)
        end
    end
end
